...
to [[haskell-cabal][Cabal]] or [[haskell-stack][Stack]], exactly matching the dependencies one would use if Nix was
...
in =stack.yaml=), only Haskell.nix is supported by =nix-haskell-tags=.
...
with Nix. The best resources are the [[nixpkgs-manual][Nixpkgs]] and [[haskell.nix][Haskell.nix]] manuals. However,
...
Nix, but also explains how to run and install the =nix-haskell-tags= executable
provided by this project.

Once you have =nix-haskell-tag= available to call, you can use it to generate a
...
=nix-haskell-tags= with =--include-targets=. In our example, this would include
...
you can call =nix-haskell-tags= with the =--no-script-link=, which will create
...
expression. When this happens, you can rerun =nix-haskell-tags= to regenerate
...
and another for tags of source outside =/nix/store=. =nix-haskell-tags= does
...
The default behavior of =nix-haskell-tags= generates writable tags files. With
the =--static= switch of =nix-haskell-tags= you can generate a single read-only
...
=nix-haskell-tags= instead. On a default configuration of Emacs, you can use
...
You'll be using =nix-haskell-tags= instead. For a default configuration of Vim,
...
Tools like [[haskell-hls][Haskell Language Server (HLS)]], which implement's [[lsp][Microsoft's Language
...
It's very difficult to cover the myriad of plugins, alternative configurations,
...
Rather than use the provided =nix-haskell-tags= command-line tool, you may want
...
selected with the "nix-haskell-tags-static" and "nix-haskell-tags-dynamic"
attributes.
...
In both =nix-haskell-tags-static= and =nix-haskell-tags-dynamic= functions, the
...
This project is very similar and takes some ideas from [[thax][tek/thax]]. There's a few
...
Thax is closer to the "static" usage of =nix-haskell-tags=. Immutable tags are
...
possible. This is different from the =--static= switch of =nix-haskell-tags=.
