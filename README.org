...
** Calling =nix-haskell-tags=
   ...
   that you need the "./" prefix is needed because paths in Nix syntax must
   contain at least one slash character.
...
** The tags generation script
   ...
   generation script before calling =nix-collect-garbage=.  Alternatively, you
   can call =nix-haskell=tags= with the =--no-script-link= which will create and
   run the script, but not link it as =run/tags-generate= or set it up as a GC
   root.
