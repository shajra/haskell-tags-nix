...
```
    ...
    /nix/store/4xxn6rbz4m64glj63cq5vl3q3535wp5w-nix-haskell-tags
...
You can run a command from a package in a Nix expression with `nix run`. For instance, to get the help message for the `nix-haskell-tags` executable provided by the "nix-haskell-tags" package selected by the "nix-haskell-tags-exe" attribute path, we can call the following:
